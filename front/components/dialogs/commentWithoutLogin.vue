<template>
	<v-dialog v-model="visible" content-class="commentWithoutLoginDialog" persistent max-width="500px">
		<!-- Popup Header -->
		<div class="header">
			<v-icon text class="cancelIcon" size="20" @click="closeDialog()">fas fa-times</v-icon>
		</div>
		...
		<div class="body">
			...
			<div v-if="successMessage == ''">
				...
				<form id="comment-on-idea-without-login-form" @submit.prevent="signup">
					<!-- Text Fields -->
					<div>
						<v-text-field v-model="form.email" v-validate="'required|email|max:100'" class="emailInput" single-line flat prepend-inner-icon="email" :error-messages="errors.collect('email')" data-vv-name="email" label="Enter email"></v-text-field>
					</div>

					<!-- Submit Buttons -->
					<div class="specialButton submitBtn">
						<v-btn :loading="loginInProgress" type="submit" class="specialButton shareBtn" form="comment-on-idea-without-login-form">SEND</v-btn>
					</div>
				</form>
			</div>
